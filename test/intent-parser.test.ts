...
describe("Intent Parser - Positive Cases", () => {
  ...
  for (const primitive of primitives) {
    const testInputs =
      nlInputs.positive_cases[
        primitive as keyof typeof nlInputs.positive_cases
      ];
    ...
    for (let i = 0; i < testInputs.length; i++) {
      ...
      test(`${primitive} - Input ${i + 1}: "${input}"`, () => {
        ...
        assertDefined(
          ...
          "Intent should include reasoning",
        );
      });
    }
  }
  ...
  for (let i = 0; i < nlInputs.negative_cases.length; i++) {
    ...
    test(`Negative Case ${i + 1}: "${input}"`, () => {
      ...
      if (result.intent) {
        ...
        assertDefined(
          result.error,
          "Should include error message when no intent found",
        );
      }
    });
  }
...
describe("Intent Parser - Parameter Extraction", () => {
  ...
  for (const testCase of nlInputs.parameter_extraction) {
    test(`Extract params from: "${testCase.input}"`, () => {
      ...
      assertDefined(
        result.intent,
        `Failed to parse intent for "${testCase.input}"`,
      );
...
describe("Intent Parser - Reasoning Generation", () => {
  ...
  test("Reasoning explains context matches", () => {
    ...
    expect(
      reasoning.includes("check") ||
        reasoning.includes("quality") ||
        reasoning.includes("security"),
    ).toBeTruthy();
  });
