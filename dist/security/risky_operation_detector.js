 ...
 */
export var GitOperationTier;
(function (GitOperationTier) {
    GitOperationTier["ALWAYS_BLOCKED"] = "always_blocked";
    GitOperationTier["REQUIRES_CONFIRMATION"] = "requires_confirmation";
    GitOperationTier["SAFE"] = "safe";
})(GitOperationTier || (GitOperationTier = {}));
/**
...
export class RiskyOperationDetector {
     ...
     */
    formatBlockedMessage(operations) {
...
These operations require manual execution outside of AI agent workflows.`;
     ...
     */
    formatConfirmationMessage(operations) {
        const opList = operations.map((op) => op.operation).join(", ");
        const risks = operations
            .map((op) => `  • ${op.risk_description}`)
            .join("\n");
...
A safety checkpoint will be created automatically before execution.`;
     ...
     */
    formatConfirmationMetadata(operations) {
        const opList = operations.map((op) => op.operation).join(", ");
        return {
...
//# sourceMappingURL=risky_operation_detector.js.map