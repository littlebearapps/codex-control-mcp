 ...
 */
export function extractMetadata(output, exitCode, threadId, taskId) {
    const metadata = {
 ...
 */
function extractThreadInfo(output, threadId) {
    const threadInfo = {
        thread_id: threadId,
    };
 ...
 */
function extractErrorContext(output, exitCode) {
    const errorContext = {};
    ...
    const seen = new Set(); // Track unique locations
    let match;
 ...
 */
function generateSuggestions(errorContext, output, exitCode) {
    const suggestions = [];
...
//# sourceMappingURL=metadata_extractor.js.map