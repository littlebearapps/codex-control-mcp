 ...
 */
async function runGit(command, workingDir) {
    try {
 ...
 */
export async function verifyGitOperations(workingDir, taskDescription) {
    const result = {
        branchExists: false,
        commitExists: false,
        filesStaged: false,
    ...
    if (branchCheck.success) {
        ...
        if (expectations.expectBranch) {
            ...
            if (!result.branchExists) {
                result.errors.push(`Branch not created: Expected \`${expectations.expectBranch}\`, still on \`${result.actualBranch}\``);
                result.recommendations.push(`Create branch manually: \`git checkout -b ${expectations.expectBranch}\``);
            }
        }
    }
    else {
    ...
    if (commitCheck.success && commitCheck.stdout) {
        ...
        if (expectations.expectCommit && expectations.expectCommitMessage) {
            ...
            const messageMatches = message.includes(expectations.expectCommitMessage.substring(0, 20));
            result.commitExists = messageMatches;
            if (!messageMatches) {
                result.errors.push(`Commit not found: Expected message containing "${expectations.expectCommitMessage.substring(0, 40)}...", got "${message}"`);
                result.recommendations.push(`Create commit manually: \`git commit -m "${expectations.expectCommitMessage}"\``);
            }
        }
    }
...
export function formatGitVerification(verification) {
    ...
    if (verification.expectedCommitMessage) {
        ...
        lines.push(`ℹ️ Latest commit: "${verification.actualCommitMessage.substring(0, 60)}"`);
    }
    // Staging status
    ...
    if (verification.unstagedFiles.length > 0) {
        lines.push(`⚠️ Files unstaged: ${verification.unstagedFiles.length} file(s)`);
    }
    // Errors
