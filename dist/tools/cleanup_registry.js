...
export class CleanupRegistryTool {
    ...
    async execute(input) {
        ...
        if (input.format === "json") {
            ...
            const stuckCandidates = all.filter((t) => (t.status === "pending" || t.status === "working") &&
            ...
            const json = {
                ...
                data: {
                    ...
                    old_tasks: {
                        ...
                        oldest_age_hours: oldCandidates.length > 0
                            ? Math.max(...oldCandidates.map((t) => Math.floor((now - (t.completedAt || now)) / 3600000)))
                            : 0,
                    },
