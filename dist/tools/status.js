...
export class StatusTool {
  ...
  async execute(input) {
    ...
    const tasks = showAll
      ? localTaskRegistry.getAllTasks()
      : allTasksInWorkingDir;
    if (tasks.length === 0) {
      message += showAll
  ...
  static getSchema() {
    return {
      name: "codex_status",
      description:
        "Check unified Codex execution status: active processes (PIDs, queue, concurrency) AND task registry (running, completed, failed tasks). Consolidates process manager and persistent task tracking in one view.",
      inputSchema: {
        type: "object",
        properties: {
          workingDir: {
