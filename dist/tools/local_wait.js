...
export class LocalWaitTool {
    ...
    async execute(input) {
        ...
        if (!task) {
            if (preferred === 'json') {
                const json = {
                    ...
                    status: 'error',
                    meta: {},
                    error: { code: 'NOT_FOUND', message: 'Task not found', details: { task_id: input.task_id }, retryable: false },
                };
        ...
        if (preferred === 'json') {
            ...
            const resultObj = task.result ? (() => { try {
            ...
            if (json.meta.exit_code === undefined)
                delete json.meta.exit_code;
            if (includeOutput) {
                json.data.output = {
                    included: true,
                    stdout: tOut.text,
