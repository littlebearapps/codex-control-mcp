...
export class CloudWaitTool {
  ...
  async execute(input: CloudWaitInput) {
    ...
    if (!task) {
      if (preferred === 'json') {
        const json = {
          ...
          meta: {},
          error: { code: 'NOT_FOUND' as const, message: 'Task not found in cloud registry', details: { task_id: input.task_id }, retryable: false },
        };
    ...
    if (preferred === 'json') {
      const state: 'completed' | 'failed' | 'cancelled' | 'timeout' = timedOut ? 'timeout' : (task.status as any);
