 ...
 */
async function checkCloudStatus(taskId: string): Promise<{
  status: string;
  details?: any;
}> {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          timeout_sec: {
            type: 'number',
            description:
              'Max seconds to wait (default: 300, i.e., 5 minutes). Server caps this at 30 minutes for cloud tasks.',
          },
          poll_interval_sec: {
            type: 'number',
            description:
              'Initial polling interval in seconds (default: 10). Server adjusts with backoff for efficiency.',
          },
        },
        required: ['task_id'],
      },
