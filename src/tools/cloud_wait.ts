...
export interface CloudWaitInput {
  task_id: string;
  timeout_sec?: number;
  include_output?: boolean; // Not applicable for cloud (no logs), ignored
...
export class CloudWaitTool {
  static getSchema() {
    return {
      ...
      description:
        "Wait for a cloud task to complete and return a structured summary. Use _codex_cloud_results to view details/PR links in Web UI.",
      inputSchema: {
        type: "object",
        properties: {
          task_id: {
            type: "string",
            description: "Cloud task ID (T-cloud-...)",
          },
          timeout_sec: {
            type: "number",
          ...
          include_output: {
            type: "boolean",
            description: "No effect for cloud; logs not available via registry",
            default: false,
          },
          format: {
            type: "string",
  ...
  async execute(input: CloudWaitInput) {
    const preferred = input.format || "markdown";
    const timeoutSec =
      typeof input.timeout_sec === "number" ? input.timeout_sec : 600;
    ...
    if (!task) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `❌ Task not found in cloud registry: ${input.task_id}`,
          },
        ],
        isError: true,
      };
      ...
      const json: any = {
        ...
        meta: {
          ...
          duration_ms: Math.max(
            0,
            completedTs.getTime() - new Date(task.timestamp).getTime(),
          ),
        },
        data: {
    ...
    if (timedOut) {
      return {
        content: [
          {
            type: "text",
            text: `⏱️ Timed out waiting for cloud task ${input.task_id} after ${timeoutSec}s.`,
          },
        ],
        isError: true,
      };
    ...
    return {
      content: [
        {
          type: "text",
          text: `✅ Cloud task ${input.task_id} ${task.status}. View results: ${url}`,
        },
      ],
    };
  }
