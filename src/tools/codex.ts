...
import {
  extractMetadata,
  type CodexMetadata,
} from "../utils/metadata_extractor.js";
  ...
  try {
    ...
    if (!primitives) {
      return {
        ...
        action: "none",
        user_message:
          "Primitive tools not available (server initialization error)",
        error: {
...
function convertPrimitiveResult(
  ...
  const textContent =
    primitiveResult.content?.[0]?.text ||
  ...
  try {
    const exitCode =
      primitiveResult.exit_code ||
      primitiveResult.exitCode ||
      (isError ? 1 : 0);
    const threadId =
      primitiveResult.thread_id || primitiveResult.threadId || routing.threadId;
...
function mapIntentToAction(
  intentType: string,
): "run" | "check" | "wait" | "cancel" | "setup" | "results" | "none" {
  switch (intentType) {
    case "execute":
      return "run";
    case "status":
  ...
  inputSchema: {
    ...
    properties: {
      ...
      hints: {
        ...
        properties: {
          operation: {
            ...
            description:
              "The operation type (bypasses natural language parsing).",
          },
          taskId: {
            type: "string",
            description:
              "Explicit task ID (format: T-local-abc123 or T-cloud-def456).",
          },
          mode: {
            type: "string",
      ...
      context: {
        type: "object",
        description:
          "Optional: Execution context (working directory, repo root).",
        properties: {
          working_dir: {
            type: "string",
      ...
      dry_run: {
        type: "boolean",
        description:
          "If true, route the request but do not execute. Useful for testing.",
      },
      explain: {
        type: "boolean",
        description:
