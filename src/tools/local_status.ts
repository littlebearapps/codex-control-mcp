...
export interface LocalStatusInput {
  ...
  showAll?: boolean; // Show all tasks including completed
}
...
export class LocalStatusTool {
  async execute(input: LocalStatusInput = {}): Promise<LocalStatusResult> {
  ...
  private getElapsedTime(submittedAt: string): string {
  ...
  static getSchema() {
    return {
      ...
      description:
        'Check status of async local Codex tasks. Shows running and recently completed tasks in current directory.',
      inputSchema: {
        type: 'object',
        properties: {
          taskId: {
