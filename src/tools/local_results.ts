...
export class LocalResultsTool {
  async execute(input: LocalResultsInput): Promise<LocalResultsResult> {
    ...
    if (input.format === "json") {
      ...
      const rawOut = (resultData.finalOutput ||
        resultData.stdout ||
        "") as string;
      ...
      const metadata = extractMetadata(
        stdout + (stderr ? `\n${stderr}` : ""),
        resultData.exitCode ?? (success ? 0 : 1),
        task.threadId,
        input.task_id,
      );
      ...
      if (includeOutput) {
        ...
        if (tOut.truncated || tErr.truncated) {
          (json.data as any).output.original_size =
            (tOut.original || 0) + (tErr.original || 0);
        }
      } else {
        (json.data as any).output = {
    ...
    if (resultData.finalOutput) {
      ...
      if (wasTruncated) {
        ...
        const truncatedLines = output
          .substring(40000, output.length - 5000)
          .split("\n").length;
      ...
      inputSchema: {
        ...
        properties: {
          ...
          format: {
            ...
            default: "markdown",
            description:
              'Response format: "json" for structured data (97% token reduction), "markdown" for human-readable (default).',
          },
          include_output: {
            type: "boolean",
            description:
              "Include stdout/stderr in response (JSON format only). Default: false for success, true for failure.",
          },
          max_output_bytes: {
            type: "number",
            description:
              "Maximum bytes of output to include (default: 65536). Output truncated if exceeded.",
          },
        },
        required: ["task_id"],
      },
