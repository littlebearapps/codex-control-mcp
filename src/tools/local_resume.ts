...
export class LocalResumeTool {
      ...
      inputSchema: {
        ...
        properties: {
          threadId: {
            type: "string",
            description:
              "The thread ID to resume (from previous codex_local_exec result)",
          },
          task: {
            type: "string",
  ...
  async execute(
    ...
    console.error(
      "[LocalResume] ==================== EXECUTE START ====================",
    );
    console.error(
      "[LocalResume] Resuming thread with:",
    ...
    try {
      ...
      if (riskyOps.length > 0) {
        ...
        if (highestTier === GitOperationTier.ALWAYS_BLOCKED) {
          ...
          console.error(
            "[LocalResume] ❌ BLOCKED - Destructive git operation detected",
          );
          return {
            content: [
              {
        ...
        if (
          ...
          console.error(
            "[LocalResume] ⚠️ RISKY operation detected, user confirmation required",
          );
          return {
            content: [
              {
        ...
        if (validated.allow_destructive_git) {
          console.error(
            "[LocalResume] ⚠️  User confirmed risky operation, creating safety checkpoint...",
          );
          ...
          console.error(
            "[LocalResume] ✅ Safety checkpoint created:",
            checkpoint.safety_branch,
          );
          console.error(
            "[LocalResume] Recovery instructions will be included in output",
          );
        }
      }
      ...
      console.error(
        "[LocalResume] *** STARTING ASYNC EXECUTION (NON-BLOCKING) ***",
      );
      ...
      thread
        .runStreamed(validated.task, runOptions)
        .then(async ({ events }) => {
          console.error(
            `[LocalResume:${validated.threadId}] Background execution started`,
          );
          ...
          const hardTimeoutTimer = setTimeout(() => {
            if (!timedOut) {
              timedOut = true;
              console.error(
                `[LocalResume:${validated.threadId}] ⏱️ HARD TIMEOUT after ${hardTimeoutMs / 1000}s`,
              );
            }
          }, hardTimeoutMs);
          ...
          const idleCheckInterval = setInterval(() => {
            ...
            if (idleTime > idleTimeoutMs && !timedOut) {
              timedOut = true;
              console.error(
                `[LocalResume:${validated.threadId}] ⏱️ IDLE TIMEOUT after ${idleTimeoutMs / 1000}s of inactivity`,
              );
          ...
          try {
            for await (const event of events) {
              ...
              if (timedOut) {
                console.error(
                  `[LocalResume:${validated.threadId}] Stopping iteration - timed out`,
                );
                break;
              }
              ...
              eventCount++;
              console.error(
                `[LocalResume:${validated.threadId}] Event ${eventCount}:`,
                event.type,
              );
            ...
            console.error(
              `[LocalResume:${validated.threadId}] ✅ Execution complete, ${eventCount} events processed`,
            );
            ...
            await sendProgressNotification(
              ...
              createCompletionNotification(
                validated.threadId,
                "Codex SDK thread resumed - execution complete",
              ),
              `LocalResume:${validated.threadId}`,
            );
          } catch (streamError) {
            ...
            console.error(
              `[LocalResume:${validated.threadId}] ❌ Error during execution:`,
              streamError,
            );
          }
        })
        .catch((error) => {
          console.error(
            `[LocalResume:${validated.threadId}] ❌ Fatal error:`,
            error,
          );
        });
      ...
      console.error(
        "[LocalResume] ✅ Returned thread ID immediately (non-blocking)",
      );
      console.error(
        "[LocalResume] ==================== EXECUTE END (ASYNC) ====================",
      );
      ...
      console.error(
        "[LocalResume] Error type:",
        error instanceof Error ? "Error" : typeof error,
      );
      console.error(
        "[LocalResume] Error message:",
        error instanceof Error ? error.message : String(error),
      );
      console.error(
        "[LocalResume] Error stack:",
