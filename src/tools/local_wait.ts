...
export class LocalWaitTool {
  static getSchema() {
    return {
      ...
      description: 'Wait for a local task to complete and return full results. This blocks until the task reaches a terminal state or the timeout is reached.',
      inputSchema: {
        type: 'object',
        properties: {
          task_id: { type: 'string', description: 'Local task ID (T-local-...)' },
          timeout_sec: { type: 'number', description: 'Maximum seconds to wait (default 300)', default: 300 },
          include_output: { type: 'boolean', description: 'Include full stdout/stderr output (default false; included automatically on failure).', default: false },
          max_output_bytes: { type: 'number', description: 'Max bytes for output (default 65536).', default: 65536 },
  ...
  async execute(input: LocalWaitInput) {
    ...
    const timeoutSec = typeof input.timeout_sec === 'number' ? input.timeout_sec : 300;
    ...
    if (!task) {
      if (preferred === 'json') {
        const json = {
          ...
          meta: {},
          error: { code: 'NOT_FOUND' as const, message: 'Task not found', details: { task_id: input.task_id }, retryable: false },
        };
    ...
    if (preferred === 'json') {
      ...
      const resultObj = task.result ? (() => { try { return JSON.parse(task.result as string); } catch { return {}; } })() : {};
      ...
      const json: any = {
        ...
        data: {
          ...
          state,
          summary: state === 'completed' ? 'Task completed successfully' : state === 'timeout' ? 'Task timed out while waiting' : state === 'cancelled' ? 'Task cancelled' : `Task failed${task.error ? `: ${task.error}` : ''}`,
          metadata,
        },
      };
      ...
      if (includeOutput) {
        ...
        json.data.output = { included: false, reason: 'Output excluded by default (use include_output=true)', truncated: false, max_bytes: 0 };
      }
    ...
    if (timedOut) {
      return { content: [{ type: 'text', text: `⏱️ Timed out waiting for task ${input.task_id} after ${timeoutSec}s.` }], isError: true };
    }
