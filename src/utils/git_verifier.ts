  ...
  if (statusCheck.success) {
    ...
    if (expectations.expectFilesStaged && expectations.expectFilesStaged.length > 0) {
      const missingFiles = expectations.expectFilesStaged.filter(
        f => !result.stagedFiles.includes(f) && !result.stagedFiles.some(sf => sf.includes(f))
      );

      if (missingFiles.length > 0) {
        result.errors.push(
          `Files not staged: ${missingFiles.join(', ')}`
        );
        result.recommendations.push(
 ...
 */
export function formatGitVerification(verification: GitVerificationResult): string {
  const lines: string[] = [];
