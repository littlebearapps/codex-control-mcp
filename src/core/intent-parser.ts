...
export interface PrimitiveIntent {
  primitive: string; // e.g., '_codex_local_run'
  confidence: number; // 0-100
  extractedParams: Record<string, any>;
  reasoning?: string; // Why this primitive was selected
}

export interface IntentParseResult {
  intent: PrimitiveIntent | null;
  alternatives: PrimitiveIntent[]; // Other possible matches
...
export class IntentParser {
   ...
   */
  private scorePrimitive(input: string, pattern: typeof PRIMITIVE_PATTERNS[keyof typeof PRIMITIVE_PATTERNS], primitiveN: string): number {
    let score = 0;
    ...
    if (hasLocalTaskId) {
      if (isLocalPrimitive) score += 40; // Strong boost for local primitive
      if (isCloudPrimitive) score -= 30; // Strong penalty for cloud primitive
    }
    if (hasCloudTaskId) {
      if (isCloudPrimitive) score += 40; // Strong boost for cloud primitive
      if (isLocalPrimitive) score -= 30; // Strong penalty for local primitive
    }
    ...
    if (hasExplicitCloud) {
      if (isCloudPrimitive) score += 25; // Boost cloud primitives when "in the cloud" mentioned
      if (isLocalPrimitive) score -= 20; // Penalty for local primitives
    }
    if (hasExplicitLocal) {
      if (isLocalPrimitive) score += 25; // Boost local primitives when "locally" mentioned
      if (isCloudPrimitive) score -= 20; // Penalty for cloud primitives
    }
   ...
   */
  private extractParameters(input: string, primitive: string): Record<string, any> {
   ...
   */
  private generateReasoning(input: string, primitive: string, pattern: typeof PRIMITIVE_PATTERNS[keyof typeof PRIMITIVE_PATTERNS]): string {
